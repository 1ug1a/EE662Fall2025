...
class SensorNode(wsn.Node):
    ...
    def run(self):
        ...
        if config.SIM_KILL_NODES == True:
            if self.id in NODES_TO_KILL:
                self.set_timer('TIMER_DIE', config.SIM_KILL_TIME)
    ...
    def send_network_request(self, dest, use_mesh = True):
        ...
        """
        self.log("📡 %s: Sending NETWORK_REQUEST to destination %s, role: %s" % (str(self.addr), str(dest), str(self.role)))
    ...
    def send_network_update(self):
        ...
        #self.log("🐞 [DEBUG]: %s: Role: %s / Parent ID: %s / Neighbors: %s" % (self.addr, self.role, self.parent_gui, list(self.neighbors.keys())))
        addr = None
    ...
    def on_timer_fired(self, name, *args, **kwargs):
        ...
        match name:
            ...
            case 'TIMER_JOIN_REQUEST_RECV':  # after collecting join requests for some time, send a network request to root
                ...
                self.set_timer("TIMER_NETWORK_REQUEST", 120+round(self.id/100)*120)
        ...
        if pck['dest'] == wsn.BROADCAST_ADDR or (self.ch_addr is not None and pck['dest'] == self.ch_addr) or (self.addr is not None and pck['dest'] == self.addr):
            ...
            if self.addr is not None:
                if pck['type'] == "CLUSTER_ALIVE":
                    self.route_and_forward(pck, use_mesh = False)
                else:
    ...
    def select_and_nominate(self, addr):
        ...
        self.log("🚀 %s: Received NETWORK_RESPONSE. Promoting node %s to CLUSTER_HEAD with address %s" % (str(self.addr), max_distance_gui, str(addr)))
    ...
    def check_neighbors(self):
        ...
        if will_be_removed:
            #self.log("🧹 [DEBUG]: %s: Role: %s / To be removed: %s" % (self.addr, self.role, list(will_be_removed)))
            pass
    ...
    def check_clusters(self):
        ...
        if will_be_removed:
            self.log("🧹 %s: Cluster(s) %s inactive. Clearing..." % (str(self.ch_addr), will_be_removed))
    ...
    def route_and_forward(self, pck, use_mesh = True):
        ...
        if config.SIM_MESH_ROUTING == True and use_mesh == True:
        ...
        if recolor:
            match new_role:
                ...
                case Roles.CLUSTER_HEAD:
                    ...
                    self.draw_tx_range()
                case Roles.ROOT:
                    self.scene.nodecolor(self.id, 0.90, 0.12, 0.60) # pink
                    self.draw_tx_range()
